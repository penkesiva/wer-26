...
3. **Active context-matching assistant (generalized)**  
   A **proactive mode** where the phone holds the user's current context/intent (from calendar, task list, itinerary, shopping list, health goals) and glasses continuously perceive the environment. Phone's on-device vision extracts relevant text or objects from the scene and **matches against user context in real time** — alerting the user without them asking.

   **Pattern:** user intent (phone) + environment perception (glasses) → match/mismatch → proactive cue.

   **Practical examples:**

   - **Travel:** Walking through airport — glasses see "Gate C7", itinerary says B14 → cue: "Wrong gate, B14 is back the other way."
   - **Grocery:** Glasses see a shelf label — shopping list has "oat milk" → cue: "That's the one, grab the 32oz."
   - **Calendar:** Glasses see user still at a coffee shop — meeting starts in 4 min → cue: "Your next meeting is in 4 min, Conference Room 3."
   - **Home repair:** Glasses see a hardware shelf — parts list has "M6 bolt" → cue: "That's the right size."

   *Key requirements:* context source integration (calendar/email/tasks), scene text/object extraction, low-latency local matching, intent detection to suppress false alerts.
